...
```mermaid
flowchart TD
  A[전원 ON] --> B[초기화: ADC 수위센서 / LCD / LED / 부저 / 서보 / IR]
  B --> C{모드 선택}
  C -->|AUTO| D[AUTO 모드 루프]
  ...
  F --> G[표시: LCD에 수위/상태 또는 시간]
  F --> H[LED 표시: 정상(초록) / 주의(노랑) / 위험(빨강)]
  F --> I{FLOOD 상태?}
  I -->|YES| J[서보: 차수막 상승]
  I -->|NO| K{안전 수위로 복귀?}
  K -->|YES| L[서보: 차수막 하강]
  ...
  %% 히스테리시스(임계값 구간)로 반복 동작 방지
  %% FLOOD 진입 임계값(HIGH), 복귀 임계값(LOW) 같은 형태로 운용
